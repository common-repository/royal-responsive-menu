...
class ResponsiveMenuSelectOptions{
	...
	function initializeMenuPage(){
		...
		extract( wp_parse_args( $this->config, array(
			...
			'page_title'	=>	'Royal Panel Options',
			'menu_title'	=>	'Royal Panel',
		...
		switch( $this->menu_type ){
			...
			case 'submenu_page':
				...
				$this->menu_page = add_submenu_page( 
					$this->parent_slug, 
					$this->page_title , //'royaltechnologies', 
					$this->menu_title , //'royaltechnologies', 
					$this->capability, 
					$this->menu_slug, 
					array( $this, 'show' ) );
	...
	function loadCSS(){
		...
		wp_enqueue_style('royaltechnologies-css', 	$tmp.'/royaltechnologies.css', 	false, '1.1', 'all');
		
		do_action( 'royaltechnologies_load_css_'.$this->id );
	
	}
	function loadJS(){
		$tmp = plugins_url().'/'.str_replace( basename( __FILE__ ),"",plugin_basename( __FILE__ ));
		wp_enqueue_script( 'jquery' );	// Load jQuery
		wp_enqueue_script( 'royaltechnologies-js', 	$tmp.'/royaltechnologies.js', 	false, '1.1', 'all');
		
		do_action( 'royaltechnologies_load_js_'.$this->id );
	}
	...
	function show(){
		...
		?>
		<div class="wrap Royal-controlPanel">
			
			<?php do_action( 'royaltechnologies_before_settings_panel_'.$this->id ); ?>
	
			<div class="Royal-settings-panel">
				<div class="Royal-nav">
					...
					<?php
						foreach($this->panels as $panel_id => $config){
							?>
						<li><a href="#Royal-<?php echo $panel_id; ?>"><?php echo $config['name']; ?></a></li>
							<?php
						}	
						?>
					</ul>
					...
					<div class="Royal-nav-footer">
						...
						<a href="http://royaltechbd.com" class="Royal-attribution" target="_blank">by Royal Technologies</a>
					</div>
					
				</div>
				<div class="Royal-panels">
					<form method="post" id="Royal-options">
						<?php
						...
						foreach($this->panels as $panel_id => $config){		
							...
							<div id="Royal-<?php echo $panel_id; ?>" class="Royal-panel">
								
								<?php if( $this->notification && $panel_id == $start_panel ): ?>
									<div class="Royal-infobox"><?php echo $this->notification; ?></div>					
								<?php endif; ?>
								
								<?php if( $this->warning && $panel_id == $start_panel ): ?>
									<br/>
									<div class="Royal-infobox Royal-infobox-warning"><?php echo $this->warning; ?></div>					
								<?php endif; ?>
								...
								<input type="submit" name="<?php echo $this->id; ?>-Royalops_submit" value="Save All Settings" class="button save-button"/>
						...
						<?php wp_nonce_field( $this->options_key , '_royaltechnologies-nonce' ); ?>
			...
			</div> <!-- end Royal-settings-panel -->
		</div> <!-- end Royal-controlPanel -->	
	...
	function showAdminOption($id, $config, $class=''){
		...
		$html = '<div id="container-'.$id.'" class="Royal-admin-op container-type-'.$type.' '.$class.'">';
		...
		$title = '<label class="Royal-admin-op-title" for="'.$id.'">'.$title.'</label>';
		$desc = empty($desc) ? '' : '<span class="Royal-admin-op-desc">'.$desc.'</span>';
		$units = '<span class="Royal-admin-op-units">'.$units.'</span>';
		...
		switch($type){
			...
			case 'checklist':
				...
				$html.= '<label class="Royal-admin-op-title">'.$config['title'].'</label>'; //$title;
				$html.= $desc;
				$html.= '<div class="Royal-admin-checklist">';
				...
				if(is_array($ops)){
					...
					foreach($ops as $opVal => $op){
						...
						$input_id = $id.'-'.$k;
						$html.= '<label class="Royal-admin-op-title" for="'.$input_id.'">'.$op.'</label> <input type="checkbox" value="'.$opVal.'" '.$checked.' name="'.$id.'[]" id="'.$input_id.'" />';
						$k++;
					}
				}
			...
			case 'radio':
				...
				if(is_array($ops)){
					foreach($ops as $opVal => $op){
						...
						$html.= '<div class="Royal-admin-op-radio">';
			...
			case 'infobox':
				$html.= '<div class="Royal-infobox '.$special_class.'">';
			...
			case 'reset':
				...
				$html.= '<input type="submit" id="'.$id.'" name="'.$this->id.'-reset-options" value="Reset" class="button reset-button" 
							onClick="return confirm(\'WARNING: Are you sure you want to reset all RoyalMenu options?\');" />';
				$html.= $desc;
	 ...
	 * @param special_class String - the class to add to the dialog, like 'Royal-infobox-warning'
	 */
	 ...
	 * @param special_class - the class to add to the dialog, like 'Royal-infobox-warning'
	 */
	...
	function updateSettings(){
		...
		if( isset( $_POST[$this->id.'-reset-options'] ) ){
			...
			if( check_admin_referer( $this->options_key , '_royaltechnologies-nonce' ) ){
		...
		if( !isset( $_POST[$this->id.'-Royalops_submit'] ) ) return false;
		...
		do_action( 'royaltechnologies_update_settings_'.$this->id , $saveOps );
		...
		$this->notification = "Royal Responsive Menu Settings saved!";
